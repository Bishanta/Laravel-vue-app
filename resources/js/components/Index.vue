...
<div>
  ...
  <nav class="nav-container" >
    <div class="container">
      <div class="logo">
        <span class="price">Copyright @2020 made by Bishant</span>
      </div>
      <div class="links">
          <router-link
            class=""
        ...
        <a @click="logout" v-if="isLoggedIn"> <i class="fas fa-user-check"></i> &nbsp; Logout</a>
      </div>
    ...
    <div class="index-container">
        <div class="item-container">
            <div class="item-type"><span>CD</span></div>
                    ...
                    <div class="item-content" v-for="(product,index) in cdPaginate" :key="index">
                        <div class="item-select">
                        <div class="product-detail-box">
                            <span class="item-title">{{product.title}}</span>
                            <span class="item-details">{{product.firstName}} &nbsp; {{product.mainName}}</span>
                            <span class="item-details">${{product.price}}</span>
                            <span class="item-details">length: &nbsp; {{product.length}}</span> 
                        </div>
            ...
            <button @click="cdnextPage"><i class="fas fa-arrow-right"></i></button>
            </div>
        ...
        <div class="item-container">
            <div class="item-type"><span>BOOK</span></div>
                    ...
                    <div class="item-content" v-for="(product,index) in bookPaginate" :key="index">
                        <div class="item-select">
                        <div class="product-detail-box">
                            <span class="item-title">{{product.title}}</span>
                            <span class="item-details">{{product.firstName}} &nbsp; {{product.mainName}}</span>
            ...
            <div class="item-select mt-20">
            <button @click="bookprevPage"><i class="fas fa-arrow-left"></i></button> 
            <button @click="booknextPage"><i class="fas fa-arrow-right"></i></button>
            </div>
            ...
            <div class="item-type"><span>LOGIN</span></div>
                    ...
                    <form action="#" @submit.prevent="handleLogin">
                            ...
                            <input  type="email" name="email" class="form-control" v-model="formData.email" placeholder="Email Address">
                                <p style="color:red;margin: 0px 16px;" v-if="errors.email">{{errors.email[0]}}</p>

                            <input type="password" name="password" class="form-control" v-model="formData.password" placeholder="Password">
                                <p style="color:red;margin: 0px 16px;" v-if="errors.password">{{errors.password[0]}}</p>
    ...
    export default {
        ...
        methods: {
            ...
            booknextPage:function() {
                if((this.bookcurrentPage*this.bookpageSize) < this.sortedBooks.length) this.bookcurrentPage++;
            },
            bookprevPage:function() {
                if(this.bookcurrentPage > 1) this.bookcurrentPage--;
            },
            cdnextPage:function() {
                if((this.cdcurrentPage*this.cdpageSize) < this.sortedCds.length) this.cdcurrentPage++;
            },
            cdprevPage:function() {
        ...
        beforeMount() {
                ...
                axios.get('api/products_all')
                .then(response => {
                console.log(response.data.filter((row)=> {
                   if(row.type.localeCompare("book") == 0) return true;
                }));

                axios.get('api/users_all_fetch')
                .then(response => {
